...
from src.db.mixin import DBMixin
from src.schemas.user import UserCreateRequest, UserUpdateRequest
from src.utils.sercurity import get_password_hash, verify_password
...
class UserService(DBMixin[User, UserCreateRequest, UserUpdateRequest]):
    ...
    def create(self, session: Session, *, obj_in: UserCreateRequest) -> User:
        db_obj = User(
            email=obj_in.email,
    ...
    def update(
            self, session: Session, *, db_obj: User, obj_in: Union[UserUpdateRequest, Dict[str, Any]]
    ) -> User:
